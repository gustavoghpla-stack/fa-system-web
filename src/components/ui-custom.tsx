...
export function PageHeader({ title, icon, children }: PageHeaderProps) {
  return (
    <div className="p-3.5 px-5 border-b border-border bg-card flex items-center gap-3 shrink-0">
      <h1 className="font-heading text-xl font-bold text-primary tracking-[1px] flex-1">
        {icon} {title}
      </h1>
      {children}
    </div>
...
export function Modal({ open, onClose, title, maxWidth = '960px', children, footer }: ModalProps) {
  ...
  return (
    ...
    >
      <div className="animate-slide-up bg-popover border border-primary/50 rounded-xl overflow-hidden flex flex-col shadow-2xl" style={{ width: '92%', maxWidth, maxHeight: '92vh' }}>
        <div className="p-3.5 px-[18px] bg-secondary border-b border-border flex items-center gap-2.5">
          <h2 className="font-heading text-lg font-bold text-primary flex-1 tracking-[0.5px]">{title}</h2>
          <button onClick={onClose} className="bg-transparent text-muted-foreground text-[22px] cursor-pointer p-[2px_8px] rounded-md transition-colors hover:text-destructive">✕</button>
        </div>
        <div className="flex-1 overflow-y-auto p-[18px]">{children}</div>
        {footer && (
          <div className="p-3 px-[18px] bg-card border-t border-border flex justify-end gap-2">
            {footer}
          </div>
        )}
      </div>
